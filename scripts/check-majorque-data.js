// Script pour vérifier les données importées pour Majorque
const fs = require('fs');
...
function main() {
  try {
    ...
    // Vérifier si les données pour Majorque existent
    if (!data.espagne || !data.espagne['majorque']) {
      console.log('Aucune donnée trouvée pour Majorque.');
      return;
    }
    ...
    console.log('Données pour Majorque :');
    console.log(`- Appartements : ${majorqueData.appartements ? majorqueData.appartements.length : 0}`);
    console.log(`- Villas : ${majorqueData.villas ? majorqueData.villas.length : 0}`);
    console.log(`- Hôtels : ${majorqueData.hotels ? majorqueData.hotels.length : 0}`);
    console.log(`- Total : ${(majorqueData.appartements?.length || 0) + (majorqueData.villas?.length || 0) + (majorqueData.hotels?.length || 0)}`);
    
    // Afficher quelques exemples d'hébergements
    if (majorqueData.appartements && majorqueData.appartements.length > 0) {
      console.log('\n=== EXEMPLE D\'APPARTEMENT ===');
      const appartement = majorqueData.appartements[0];
    ...
    if (majorqueData.villas && majorqueData.villas.length > 0) {
      console.log('\n=== EXEMPLE DE VILLA ===');
      const villa = majorqueData.villas[0];
    ...
    if (majorqueData.hotels && majorqueData.hotels.length > 0) {
      console.log('\n=== EXEMPLE D\'HÔTEL ===');
      const hotel = majorqueData.hotels[0];
    ...
    console.log('\nAPPARTEMENTS :');
    majorqueData.appartements?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nVILLAS :');
    majorqueData.villas?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nHÔTELS :');
    majorqueData.hotels?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });

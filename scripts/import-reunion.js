// Script pour importer les données d'hébergement de La Réunion depuis Apify
const fs = require('fs');
const path = require('path');
const https = require('https');

// URL de l'API Apify pour La Réunion
const APIFY_URL = 'https://api.apify.com/v2/datasets/AltPLT4Et932igL9b/items?token=apify_api_SSILb5pMEFd7M4ON7t0ViHjgvRPOjb0tOj6c';
...
function extractFeatures(item) {
  ...
  const commonFeatures = [
    ...
    { en: "Ocean view", fr: "Vue sur océan" },
  ...
  if (features.length === 0) {
    ...
    features.push("Proche des sites touristiques");
  }
...
function translateDescription(description) {
  ...
  // Ajouter un message de bienvenue personnalisé à La Réunion
  cleanDesc += "\n\nBienvenue à La Réunion, île intense aux paysages spectaculaires. Cet hébergement vous permettra de profiter pleinement des merveilles de l'île : volcans majestueux, cirques naturels, cascades rafraîchissantes et plages paradisiaques. Une expérience inoubliable vous attend dans ce cadre exceptionnel.";
...
function transformData(apifyData) {
  ...
  apifyData.forEach((item, index) => {
    ...
    const transformedItem = {
      id: item.hotelId || index + 2000000, // Utiliser l'ID de l'hôtel ou générer un ID unique
      name: item.name,
      type: type,
      ...
      price: price,
      images: item.images || [],
      location: item.address?.full || 'La Réunion, France'
    };
...
async function main() {
  try {
    console.log('Téléchargement des données depuis Apify pour La Réunion...');
    ...
    const reunionData = transformData(apifyData);
    console.log(`Données transformées: ${reunionData.appartements.length} appartements, ${reunionData.villas.length} villas, ${reunionData.hotels.length} hôtels.`);
    ...
    if (!existingData.france['la-reunion']) {
      existingData.france['la-reunion'] = {};
    }
    
    // Ajouter les données
    existingData.france['la-reunion'] = reunionData;

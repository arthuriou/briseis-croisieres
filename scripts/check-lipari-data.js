// Script pour vérifier les données importées pour Lipari
const fs = require('fs');
...
function main() {
  try {
    ...
    // Vérifier si les données pour Lipari existent
    if (!data.italie || !data.italie['lipari']) {
      console.log('Aucune donnée trouvée pour Lipari.');
      return;
    }
    
    const lipariData = data.italie['lipari'];
    
    // Afficher le nombre d'hébergements par type
    console.log('Données pour Lipari :');
    console.log(`- Appartements : ${lipariData.appartements ? lipariData.appartements.length : 0}`);
    console.log(`- Villas : ${lipariData.villas ? lipariData.villas.length : 0}`);
    console.log(`- Hôtels : ${lipariData.hotels ? lipariData.hotels.length : 0}`);
    console.log(`- Total : ${(lipariData.appartements?.length || 0) + (lipariData.villas?.length || 0) + (lipariData.hotels?.length || 0)}`);
    
    // Afficher quelques exemples d'hébergements
    if (lipariData.appartements && lipariData.appartements.length > 0) {
      console.log('\n=== EXEMPLE D\'APPARTEMENT ===');
      const appartement = lipariData.appartements[0];
    ...
    if (lipariData.villas && lipariData.villas.length > 0) {
      console.log('\n=== EXEMPLE DE VILLA ===');
      const villa = lipariData.villas[0];
    ...
    if (lipariData.hotels && lipariData.hotels.length > 0) {
      console.log('\n=== EXEMPLE D\'HÔTEL ===');
      const hotel = lipariData.hotels[0];
    ...
    console.log('\nAPPARTEMENTS :');
    lipariData.appartements?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nVILLAS :');
    lipariData.villas?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nHÔTELS :');
    lipariData.hotels?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });

// Script pour vérifier les données importées pour la Sicile
const fs = require('fs');
...
function main() {
  try {
    ...
    // Vérifier si les données pour la Sicile existent
    if (!data.italie || !data.italie['sicile']) {
      console.log('Aucune donnée trouvée pour la Sicile.');
      return;
    }
    
    const sicileData = data.italie['sicile'];
    
    // Afficher le nombre d'hébergements par type
    console.log('Données pour la Sicile :');
    console.log(`- Appartements : ${sicileData.appartements ? sicileData.appartements.length : 0}`);
    console.log(`- Villas : ${sicileData.villas ? sicileData.villas.length : 0}`);
    console.log(`- Hôtels : ${sicileData.hotels ? sicileData.hotels.length : 0}`);
    console.log(`- Total : ${(sicileData.appartements?.length || 0) + (sicileData.villas?.length || 0) + (sicileData.hotels?.length || 0)}`);
    
    // Afficher quelques exemples d'hébergements
    if (sicileData.appartements && sicileData.appartements.length > 0) {
      console.log('\n=== EXEMPLE D\'APPARTEMENT ===');
      const appartement = sicileData.appartements[0];
    ...
    if (sicileData.villas && sicileData.villas.length > 0) {
      console.log('\n=== EXEMPLE DE VILLA ===');
      const villa = sicileData.villas[0];
    ...
    if (sicileData.hotels && sicileData.hotels.length > 0) {
      console.log('\n=== EXEMPLE D\'HÔTEL ===');
      const hotel = sicileData.hotels[0];
    ...
    console.log('\nAPPARTEMENTS :');
    sicileData.appartements?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nVILLAS :');
    sicileData.villas?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nHÔTELS :');
    sicileData.hotels?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });

// Script pour vérifier les données importées pour Tenerife
const fs = require('fs');
...
function main() {
  try {
    ...
    // Vérifier si les données pour Tenerife existent
    if (!data.espagne || !data.espagne['tenerife']) {
      console.log('Aucune donnée trouvée pour Tenerife.');
      return;
    }
    ...
    console.log('Données pour Tenerife :');
    console.log(`- Appartements : ${tenerifeData.appartements ? tenerifeData.appartements.length : 0}`);
    console.log(`- Villas : ${tenerifeData.villas ? tenerifeData.villas.length : 0}`);
    console.log(`- Hôtels : ${tenerifeData.hotels ? tenerifeData.hotels.length : 0}`);
    console.log(`- Total : ${(tenerifeData.appartements?.length || 0) + (tenerifeData.villas?.length || 0) + (tenerifeData.hotels?.length || 0)}`);
    
    // Afficher quelques exemples d'hébergements
    if (tenerifeData.appartements && tenerifeData.appartements.length > 0) {
      console.log('\n=== EXEMPLE D\'APPARTEMENT ===');
      const appartement = tenerifeData.appartements[0];
    ...
    if (tenerifeData.villas && tenerifeData.villas.length > 0) {
      console.log('\n=== EXEMPLE DE VILLA ===');
      const villa = tenerifeData.villas[0];
    ...
    if (tenerifeData.hotels && tenerifeData.hotels.length > 0) {
      console.log('\n=== EXEMPLE D\'HÔTEL ===');
      const hotel = tenerifeData.hotels[0];
    ...
    console.log('\nAPPARTEMENTS :');
    tenerifeData.appartements?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nVILLAS :');
    tenerifeData.villas?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nHÔTELS :');
    tenerifeData.hotels?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });

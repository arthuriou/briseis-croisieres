// Script pour vérifier les données importées pour la Côte d'Azur
const fs = require('fs');
...
function main() {
  try {
    ...
    // Vérifier si les données pour la Côte d'Azur existent
    if (!data.france || !data.france['cote-d-azur']) {
      console.log('Aucune donnée trouvée pour la Côte d\'Azur.');
      return;
    }
    ...
    console.log('Données pour la Côte d\'Azur :');
    console.log(`- Appartements : ${coteAzurData.appartements ? coteAzurData.appartements.length : 0}`);
    console.log(`- Villas : ${coteAzurData.villas ? coteAzurData.villas.length : 0}`);
    console.log(`- Hôtels : ${coteAzurData.hotels ? coteAzurData.hotels.length : 0}`);
    console.log(`- Total : ${(coteAzurData.appartements?.length || 0) + (coteAzurData.villas?.length || 0) + (coteAzurData.hotels?.length || 0)}`);
    
    // Afficher quelques exemples d'hébergements
    if (coteAzurData.appartements && coteAzurData.appartements.length > 0) {
      console.log('\n=== EXEMPLE D\'APPARTEMENT ===');
      const appartement = coteAzurData.appartements[0];
    ...
    if (coteAzurData.villas && coteAzurData.villas.length > 0) {
      console.log('\n=== EXEMPLE DE VILLA ===');
      const villa = coteAzurData.villas[0];
    ...
    if (coteAzurData.hotels && coteAzurData.hotels.length > 0) {
      console.log('\n=== EXEMPLE D\'HÔTEL ===');
      const hotel = coteAzurData.hotels[0];
    ...
    console.log('\nAPPARTEMENTS :');
    coteAzurData.appartements?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nVILLAS :');
    coteAzurData.villas?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nHÔTELS :');
    coteAzurData.hotels?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });

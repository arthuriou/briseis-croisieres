// Script pour vérifier les données importées pour la Sardaigne
const fs = require('fs');
...
function main() {
  try {
    ...
    // Vérifier si les données pour la Sardaigne existent
    if (!data.italie || !data.italie['sardaigne']) {
      console.log('Aucune donnée trouvée pour la Sardaigne.');
      return;
    }
    
    const sardaigneData = data.italie['sardaigne'];
    
    // Afficher le nombre d'hébergements par type
    console.log('Données pour la Sardaigne :');
    console.log(`- Appartements : ${sardaigneData.appartements ? sardaigneData.appartements.length : 0}`);
    console.log(`- Villas : ${sardaigneData.villas ? sardaigneData.villas.length : 0}`);
    console.log(`- Hôtels : ${sardaigneData.hotels ? sardaigneData.hotels.length : 0}`);
    console.log(`- Total : ${(sardaigneData.appartements?.length || 0) + (sardaigneData.villas?.length || 0) + (sardaigneData.hotels?.length || 0)}`);
    
    // Afficher quelques exemples d'hébergements
    if (sardaigneData.appartements && sardaigneData.appartements.length > 0) {
      console.log('\n=== EXEMPLE D\'APPARTEMENT ===');
      const appartement = sardaigneData.appartements[0];
    ...
    if (sardaigneData.villas && sardaigneData.villas.length > 0) {
      console.log('\n=== EXEMPLE DE VILLA ===');
      const villa = sardaigneData.villas[0];
    ...
    if (sardaigneData.hotels && sardaigneData.hotels.length > 0) {
      console.log('\n=== EXEMPLE D\'HÔTEL ===');
      const hotel = sardaigneData.hotels[0];
    ...
    console.log('\nAPPARTEMENTS :');
    sardaigneData.appartements?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nVILLAS :');
    sardaigneData.villas?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nHÔTELS :');
    sardaigneData.hotels?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });

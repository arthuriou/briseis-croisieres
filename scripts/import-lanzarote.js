// Script pour importer les données d'hébergement de Lanzarote depuis Apify
const fs = require('fs');
const path = require('path');
const https = require('https');

// URL de l'API Apify pour Lanzarote
const APIFY_URL = 'https://api.apify.com/v2/datasets/n2vBI8hnsLwc2EQBh/items?token=apify_api_SSILb5pMEFd7M4ON7t0ViHjgvRPOjb0tOj6c';
...
function translateDescription(description) {
  ...
  const translations = [
    ...
    { en: "Lanzarote", fr: "Lanzarote" },
    { en: "Yaiza", fr: "Yaiza" },
    { en: "Playa Blanca", fr: "Playa Blanca" },
    { en: "Puerto del Carmen", fr: "Puerto del Carmen" },
    { en: "Arrecife", fr: "Arrecife" },
    { en: "Timanfaya National Park", fr: "Parc National de Timanfaya" },
    ...
    { en: "patio", fr: "patio" },
    { en: "volcanic landscape", fr: "paysage volcanique" }
  ];
  ...
  // Ajouter un message de bienvenue personnalisé à Lanzarote
  cleanDesc += "\n\nBienvenue à Lanzarote, l'île aux volcans. Profitez de ses paysages lunaires uniques, ses plages dorées et son architecture blanche emblématique conçue par César Manrique. Découvrez le Parc National de Timanfaya, les Jameos del Agua, et les vignobles de La Geria cultivés dans des cratères volcaniques. Une destination fascinante où nature volcanique et art se rencontrent sous le soleil des Canaries.";
...
function transformData(apifyData) {
  ...
  apifyData.forEach((item, index) => {
    ...
    const transformedItem = {
      id: item.hotelId || (9500000 + index), // Utiliser l'ID de l'hôtel ou générer un ID unique
      name: item.name,
      type: type,
      ...
      images: item.images || [],
      location: item.address?.full || 'Lanzarote, Espagne'
    };
...
async function main() {
  try {
    console.log('Téléchargement des données depuis Apify pour Lanzarote...');
    ...
    const lanzaroteData = transformData(apifyData);
    console.log(`Données transformées: ${lanzaroteData.appartements.length} appartements, ${lanzaroteData.villas.length} villas, ${lanzaroteData.hotels.length} hôtels.`);
    ...
    if (!existingData.espagne['lanzarote']) {
      existingData.espagne['lanzarote'] = {};
    }
    
    // Ajouter les données
    existingData.espagne['lanzarote'] = lanzaroteData;

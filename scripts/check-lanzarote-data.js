// Script pour vérifier les données importées pour Lanzarote
const fs = require('fs');
...
function main() {
  try {
    ...
    // Vérifier si les données pour Lanzarote existent
    if (!data.espagne || !data.espagne['lanzarote']) {
      console.log('Aucune donnée trouvée pour Lanzarote.');
      return;
    }
    ...
    console.log('Données pour Lanzarote :');
    console.log(`- Appartements : ${lanzaroteData.appartements ? lanzaroteData.appartements.length : 0}`);
    console.log(`- Villas : ${lanzaroteData.villas ? lanzaroteData.villas.length : 0}`);
    console.log(`- Hôtels : ${lanzaroteData.hotels ? lanzaroteData.hotels.length : 0}`);
    console.log(`- Total : ${(lanzaroteData.appartements?.length || 0) + (lanzaroteData.villas?.length || 0) + (lanzaroteData.hotels?.length || 0)}`);
    
    // Afficher quelques exemples d'hébergements
    if (lanzaroteData.appartements && lanzaroteData.appartements.length > 0) {
      console.log('\n=== EXEMPLE D\'APPARTEMENT ===');
      const appartement = lanzaroteData.appartements[0];
    ...
    if (lanzaroteData.villas && lanzaroteData.villas.length > 0) {
      console.log('\n=== EXEMPLE DE VILLA ===');
      const villa = lanzaroteData.villas[0];
    ...
    if (lanzaroteData.hotels && lanzaroteData.hotels.length > 0) {
      console.log('\n=== EXEMPLE D\'HÔTEL ===');
      const hotel = lanzaroteData.hotels[0];
    ...
    console.log('\nAPPARTEMENTS :');
    lanzaroteData.appartements?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nVILLAS :');
    lanzaroteData.villas?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nHÔTELS :');
    lanzaroteData.hotels?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });

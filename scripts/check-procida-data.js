// Script pour vérifier les données importées pour Procida
const fs = require('fs');
...
function main() {
  try {
    ...
    // Vérifier si les données pour Procida existent
    if (!data.italie || !data.italie['procida']) {
      console.log('Aucune donnée trouvée pour Procida.');
      return;
    }
    
    const procidaData = data.italie['procida'];
    
    // Afficher le nombre d'hébergements par type
    console.log('Données pour Procida :');
    console.log(`- Appartements : ${procidaData.appartements ? procidaData.appartements.length : 0}`);
    console.log(`- Villas : ${procidaData.villas ? procidaData.villas.length : 0}`);
    console.log(`- Hôtels : ${procidaData.hotels ? procidaData.hotels.length : 0}`);
    console.log(`- Total : ${(procidaData.appartements?.length || 0) + (procidaData.villas?.length || 0) + (procidaData.hotels?.length || 0)}`);
    
    // Afficher quelques exemples d'hébergements
    if (procidaData.appartements && procidaData.appartements.length > 0) {
      console.log('\n=== EXEMPLE D\'APPARTEMENT ===');
      const appartement = procidaData.appartements[0];
    ...
    if (procidaData.villas && procidaData.villas.length > 0) {
      console.log('\n=== EXEMPLE DE VILLA ===');
      const villa = procidaData.villas[0];
    ...
    if (procidaData.hotels && procidaData.hotels.length > 0) {
      console.log('\n=== EXEMPLE D\'HÔTEL ===');
      const hotel = procidaData.hotels[0];
    ...
    console.log('\nAPPARTEMENTS :');
    procidaData.appartements?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nVILLAS :');
    procidaData.villas?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });
    
    console.log('\nHÔTELS :');
    procidaData.hotels?.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.price !== null ? item.price + '€/nuit' : 'Prix sur demande'}, ${item.images?.length || 0} images)`);
    });

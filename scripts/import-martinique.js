// Script pour importer les données d'hébergement de la Martinique depuis Apify
const fs = require('fs');
const path = require('path');
const https = require('https');

// URL de l'API Apify pour la Martinique
const APIFY_URL = 'https://api.apify.com/v2/datasets/XbXghVIcCy6h4W9AC/items?token=apify_api_SSILb5pMEFd7M4ON7t0ViHjgvRPOjb0tOj6c';
...
function extractFeatures(item) {
  ...
  if (features.length === 0) {
    ...
    features.push("Ambiance tropicale");
...
function translateDescription(description) {
  ...
  // Ajouter un message de bienvenue personnalisé à la Martinique
  cleanDesc += "\n\nBienvenue en Martinique, île aux fleurs et terre de contrastes. Cet hébergement vous permettra de découvrir les trésors de cette île volcanique : plages de sable noir et blanc, forêts tropicales, distilleries de rhum traditionnelles et cuisine créole savoureuse. Profitez de l'art de vivre local dans ce cadre idyllique entre mer turquoise et montagnes verdoyantes.";
...
function transformData(apifyData) {
  ...
  apifyData.forEach((item, index) => {
    ...
    const transformedItem = {
      id: item.hotelId || index + 4000000, // Utiliser l'ID de l'hôtel ou générer un ID unique
      name: item.name,
      type: type,
      ...
      price: price,
      images: item.images || [],
      location: item.address?.full || 'Martinique, France'
    };
...
async function main() {
  try {
    console.log('Téléchargement des données depuis Apify pour la Martinique...');
    ...
    const martiniqueData = transformData(apifyData);
    console.log(`Données transformées: ${martiniqueData.appartements.length} appartements, ${martiniqueData.villas.length} villas, ${martiniqueData.hotels.length} hôtels.`);
    ...
    if (!existingData.france['martinique']) {
      existingData.france['martinique'] = {};
    }
    
    // Ajouter les données
    existingData.france['martinique'] = martiniqueData;

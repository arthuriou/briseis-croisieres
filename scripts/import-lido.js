// Script pour importer les données d'hébergement du Lido de Venise depuis Apify
const fs = require('fs');
const path = require('path');
const https = require('https');

// URL de l'API Apify pour le Lido de Venise
const APIFY_URL = 'https://api.apify.com/v2/datasets/7MS9taeu7d0RRc54t/items?token=apify_api_SSILb5pMEFd7M4ON7t0ViHjgvRPOjb0tOj6c';
...
function extractFeatures(item) {
  ...
  // Caractéristiques par défaut pour le Lido de Venise
  if (features.length === 0) {
    features.push("WiFi gratuit");
    features.push("Accès aux plages privées");
    features.push("Climatisation");
    features.push("Navette pour Venise");
    features.push("Vue sur la lagune");
    features.push("Architecture vénitienne");
  }
...
function translateDescription(description) {
  if (!description) return "Magnifique hébergement sur le Lido de Venise avec accès facile à la ville historique de Venise et aux plages de sable doré. Profitez d'un séjour entre culture et détente.";
  ...
  const translations = [
    ...
    { en: "Venice", fr: "Venise" },
    { en: "Lido", fr: "Lido" },
    { en: "Lido di Venezia", fr: "Lido de Venise" },
    { en: "lagoon", fr: "lagune" },
    { en: "Adriatic Sea", fr: "mer Adriatique" },
    { en: "vaporetto", fr: "vaporetto" },
    { en: "water bus", fr: "bateau-bus" },
    { en: "water taxi", fr: "taxi aquatique" },
    { en: "St. Mark's Square", fr: "place Saint-Marc" },
    { en: "Saint Mark", fr: "Saint-Marc" },
    { en: "Piazza San Marco", fr: "Place Saint-Marc" },
    { en: "Rialto Bridge", fr: "Pont du Rialto" },
    { en: "Doge's Palace", fr: "Palais des Doges" },
    { en: "Biennale", fr: "Biennale" },
    { en: "Film Festival", fr: "Festival du Film" },
    { en: "Mostra", fr: "Mostra" },
    ...
    { en: "sandy beaches", fr: "plages de sable" },
    { en: "historic center", fr: "centre historique" },
    { en: "Art Nouveau", fr: "Art Nouveau" },
    { en: "Liberty style", fr: "style Liberty" }
  ];
  ...
  // Ajouter un message de bienvenue personnalisé pour le Lido de Venise
  cleanDesc += " Bienvenue au Lido de Venise, élégante bande de terre qui sépare la lagune vénitienne de la mer Adriatique. Ce havre de paix offre une alternative raffinée à l'effervescence du centre historique de Venise, accessible en seulement quelques minutes de vaporetto. Avec ses plages de sable fin, ses somptueuses villas Art Nouveau et son ambiance Belle Époque, le Lido vous séduira par son charme discret. Hôte du prestigieux Festival International du Film de Venise, cette île allie parfaitement détente balnéaire et proximité avec les trésors culturels de la Sérénissime.";
  ...
  apifyData.forEach((item, index) => {
    try {
      ...
      const transformedItem = {
        id: item.hotelId || (8100000 + index),
        name: item.name || `Hébergement au Lido de Venise ${index + 1}`,
        type: type,
        description: translatedDescription,
        features: features,
        price: null, // Prix sur demande
        images: item.images || [],
        location: item.address?.full || 'Lido de Venise, Italie'
      };
...
async function main() {
  try {
    console.log('Téléchargement des données depuis Apify pour le Lido de Venise...');
    ...
    const lidoData = transformData(apifyData);
    console.log(`Données transformées: ${lidoData.appartements.length} appartements, ${lidoData.villas.length} villas, ${lidoData.hotels.length} hôtels.`);
    ...
    // Ajouter les données
    existingData.italie['lido'] = lidoData;
